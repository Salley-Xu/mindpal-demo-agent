#!/usr/bin/env python3
"""
v4.2 纯域训练 — 仅用对话域合成数据（600条），完全抛弃 SOS-1K。

目标：模型只学习对话域的表达方式，不再被 SOS-1K 的标签分布干扰。
"""
import os, sys
...
class Config:
    train_path = "bert_data/processed_v4/train_domain_only.jsonl"
    output_dir = "bert_data/models/v4_2_domain_only_v2"
    base_checkpoint = "bert_data/models/v4_2_multitask_calibrated/best_model/model_state.pt"

    model_name = "hfl/chinese-macbert-base"
    max_length = 128
    batch_size = 8
    learning_rate = 5e-6  # 更低 LR 防止 catastrophic forgetting
    weight_decay = 0.01
    num_epochs = 30
    warmup_ratio = 0.1
    max_grad_norm = 1.0
...
def main():
    ...
    split = int(len(train_records) * 0.8)
