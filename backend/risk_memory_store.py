...
from utils import generate_risk_event_id, generate_risk_trigger_id, generate_protective_factor_id
...
class RiskMemoryStore:
    ...
    async def _get_conn(self) -> aiosqlite.Connection:
        ...
        await conn.execute("PRAGMA busy_timeout = 5000")  # 5s 忙等待超时
    ...
    async def add_risk_event(self, event: RiskEvent) -> str:
        ...
        await conn.execute(
            ...
            """,
            (
                event.id, event.user_id, event.session_id, event.turn_id,
                event.event_time.isoformat() if isinstance(event.event_time, datetime) else event.event_time,
                event.risk_level, event.risk_score, event.subject, event.topic,
                event.summary, event.evidence_snippet,
                int(event.safety_confirmed), int(event.support_engaged),
                int(event.decayed),
            ),
        )
        await conn.commit()
    ...
    async def set_baseline(self, baseline: RiskBaseline) -> None:
        ...
        await conn.execute(
            ...
            (
                ...
                baseline.baseline_updated_at.isoformat() if isinstance(baseline.baseline_updated_at, datetime) else now,
                baseline.decay_status,
                baseline.last_high_risk_time.isoformat() if isinstance(baseline.last_high_risk_time, datetime) else None,
    ...
    async def recompute_baseline(self, user_id: str) -> RiskBaseline:
        ...
        await self.set_baseline(baseline)
        return baseline

    async def get_recent_risk_context(self, user_id: str, days: int = 14) -> Dict[str, Any]:
    ...
    async def add_protective_factor(self, factor: ProtectiveFactor) -> str:
        ...
        await conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (factor.id, factor.user_id, factor.factor, factor.source, factor.confidence, now),
        )
        await conn.commit()
        return factor.id
    ...
    def _compute_baseline_from_rows(self, rows: List[aiosqlite.Row], now: datetime) -> str:
        """
        ...
        for row in rows:
            try:
                ...
                et = datetime.fromisoformat(et_str) if isinstance(et_str, str) else et_str
        ...
        l2_in_7d = sum(1 for row in events_7d if risk_level_index(row["risk_level"]) >= 2)
        if l2_in_7d >= 2:
            return "high"
        ...
        l1_in_14d = sum(1 for row in events_14d if risk_level_index(row["risk_level"]) >= 1)
        if l1_in_14d >= 3:
            return "medium"
